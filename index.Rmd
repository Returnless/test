...
bscols(
filter_select("up", "最近", try, ~ifelse(difftime(lastdate,upload,units = "days") <= 3, "近3天",ifelse(difftime(lastdate,upload,units = "days") <= 7, "近3-7天", "7天以上"))),
filter_select("auto", "时长", try, ~ifelse(lengthnum < 300 , "5分以内",ifelse(lengthnum < 900, "5-15分", "15分以上"))),
filter_select("auto", "播放", try, ~ifelse(viewnum < 1000 , "1千以下",ifelse(lengthnum < 10000, "1千-1万", "1万以上"))))

filter_select("weishenm", "多词搜索", try, ~title)
#filter_select("h332", "搜UP主", try, ~uplink)
filter_slider("hp", "弹幕数", try, ~text, width = "100%")
#filter_slider("otl", "时间范围", try, ~upload, width = "100%")
?bscols
```
